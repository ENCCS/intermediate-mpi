...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    // FIXME initialize exposure epoch for win_n on rank 0 (target rank of RMA)
    MPI_xxx(/*  */);
    // FIXME finalize exposure epoch
    MPI_xxx(/*  */);
  } else {
    // FIXME initialize access epoch for win_n on rank > 0 (origin ranks of RMA)
    MPI_xxx(/*  */);
    // FIXME RMA with rank 0 as target process to get value of n
    MPI_xxx(/*  */);
    // FIXME finalize acces epoch
    MPI_xxx(/*  */);
  }
  ...
  if (rank > 0) {
    // FIXME initialize access epoch for win_pi on rank > 0 (origin ranks of RMA)
    MPI_xxx(/*  */);
    // FIXME RMA reduction with rank 0 as target process
    MPI_xxx(/*  */);
    // FIXME finalize acces epoch
    MPI_xxx(/*  */);
  } else {
    // FIXME initialiaze exposure epoch for win_pi on rank 0 (target rank of RMA)
    MPI_xxx(/*  */);
    // FIXME finalize exposure epoch for win_pi on rank 0 (target rank of RMA)
    MPI_xxx(/*  */);
